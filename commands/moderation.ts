...
export function ban(interaction:any, client:any, errorChannel:any, errorCmd:any) {
    ...
    guild.members.fetch(`${member.id}`).then(async (mb:any) => {
        mb.ban({ days: deleteLen, reason: reason }).then((response:Response) => {
...
export function link(interaction:any, errorChannel:any, errorCmd:any) {
    ...
    }).then(async (response:Response) => { // look a handled error!!1!1!!!!1!11!!
        interaction.reply({ content: response.status == 200 ? `Successfully linked <@${id}> to '${mc}'` : `Unable to link <@${id}> to '${mc}'`, ephemeral: true });
        if (response.status != 200) { errorCmd("Error when linking accounts", `\`\`\`\n${response}\`\`\``, errorChannel); }
    });
}
...
export function errorMessage(interaction:any, config:any) {
    ...
    switch (interaction.options.getSubcommand()) {
        case "toggle":
            ...
            interaction.reply({ content: `Toggled bot error messages to: ${config.botErrorsEnabled}`, ephemeral: true });
            break;
        case "channel":
            // set the channel
            config.botErrorChannel = interaction.options.getChannel("channel").id;
            interaction.reply({ content: `Set bot error messages to send in <#${config.botErrorChannel}>`});
            break;
    }
    return config
}