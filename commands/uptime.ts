...
export async function uptime(interaction:any, startTime:number, errorChannel:any, errorCmd:any) {
	...
	if (server == "bot") {
		...
		fetch(`https://api.prosperitymc.net/uptime/${server}`).then(async (response:Response) => {
			...
			if (uptime == -1) { uptimeStr = "This shouldn't be fucking possible"; } // Only the most polite error messages
			else if (uptime == -2) { uptimeStr = "There's been a server error, please try again later. If the issue persists, please let the mod team know."; } 
			else { uptimeStr = `${uptime}`; }
