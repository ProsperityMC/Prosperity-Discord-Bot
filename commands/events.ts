export function welcomeMessage(interaction: any, config: any) {
	switch (interaction.options.getSubcommand()) {
		case "toggle":
			...
			interaction.reply({
				content: `Welcome messages toggled to: ${!config.joinMsgEnabled}`,
				ephemeral: true
			});
		...
		case "set-channel":
			...
			interaction.reply({
				content: `Welcome messages set to send in: <#${channel.id}>`,
				ephemeral: true
			});
...
export function leaveMessage(interaction: any, config: any) {
	switch (interaction.options.getSubcommand()) {
		case "toggle":
			...
			interaction.reply({
				content: `Leave messages toggled to: ${!config.leaveMsgEnabled}`,
				ephemeral: true
			});
		...
		case "set-channel":
			...
			let channel: any = interaction.options.getChannel("channel");
			interaction.reply({
				content: `Leave messages set to send in: <#${channel.id}>`,
				ephemeral: true
			});
