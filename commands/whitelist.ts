export function whitelist(interaction:any, errorChannel:any, errorCmd:any) {
    ...
    if (mc == id) { // i.e. they're both null which means there's no one to fucking remove from the wl because the person running the command is a dipshit
    ...
    }).then(async (response:Response) => { // All cases handled, like a semi-professional programmer
        let message:string;
        if (subCmd == "add" && response.status == 200) { message = `Successfully added '${mc}' to the whitelist${id != null ? `(Linked to <@${id}>)` : ""}.`; }
        else if (subCmd == "add") { 
            message = `There was a problem when adding '${mc}' to the whitelist.`;
            errorCmd("Error adding to the whitelist", `\`\`\`\n${response}\`\`\``, errorChannel)
        }
        else if (response.status == 200) { message = `Successfully removed ${mc != null ? `'${mc}'` : `<@${id}>`} from the whitelist.`; }
        else { 
